...
To get started, let's imagine we have the following file:
...
DuckDB is actively used in data science. Perhaps the next feature of the tool is the main reason why. Let's imagine that after a file with orders for the previous year (`orders-2025.csv`)  we got orders for this year (`orders-2026.csv`):
...
And again it "just works" and produces the following results:
...
Let's say now we also get a next export, and now instead of a CSV, all we got is a JSON file called `customers.json`: 
...
You might notice that DuckDB normally "just works". We pass it whatever we expect in SQL, and it just says "Got ya". Certainly, there's a limit to that, right? Let's test it further!
...
In a normal article, I would expect that I would have to show you some trick or some technique to make it work. But today it "just works" again:
...
Let's simply join it in our final query, as if it were normal:
...
Most importantly, it is not limited to public HTTP endpoints. It's totally possible to connect to files in private accounts on file storage like S3 or Azure services! But I think the article is big enough and shows impressive enough capabilities to leave demoing such things out of scope.
